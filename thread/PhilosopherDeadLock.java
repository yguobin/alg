...
public class PhilosopherDeadLock implements Runnable {
	...
	public PhilosopherDeadLock(int number, Lock leftChopstick, Lock rightChopstick) {
		this.number = number;
...
*/
	...
	public static void main(String[] args) {
		...
		PhilosopherDeadLock[] philosophers = new PhilosopherDeadLock[n];
		Thread[] threads = new Thread[philosophers.length];
		for( int i=0; i<philosophers.length; i++ ) {
			philosophers[i] = new PhilosopherDeadLock(i, chopsticks[i%5], chopsticks[(i+1)%5]);
